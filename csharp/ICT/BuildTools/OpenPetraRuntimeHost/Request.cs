...
namespace Ict.Tools.OpenPetraRuntimeHost
{
    class Request : SimpleWorkerRequest
    {
        ...
        Object _logfileLock = new Object();
        ...
        bool TryParseRequest()
        {
            ...
            if (_host.LogfileWriter != null)
            {
                // We log all url's EXCEPT PollClientTasks because that gets requested every few seconds
                // If for some reason you need to include logging this request, comment the next line out
                // and add a line bool saveThisRequest = true;  - but DO NOT COMMIT YOUR CHANGE !!!!!!!!!
                bool saveThisRequest = (this._url != "/serverSessionManager.asmx/PollClientTasks");
            ...
            if (!_connection.IsLocal && !_host.AllowRemoteConnection)
            {
                _connection.WriteErrorAndClose(403);
                return false;
            }
            ...
            if (countNonAscii > 0)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    else
                    {
                        ...
                        expandedBytes[pos++] = (byte)IntToHex[(b & 0xf0) / 16];
