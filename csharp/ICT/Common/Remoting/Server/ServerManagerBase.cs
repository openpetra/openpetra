    ...
    public class TServerManagerBase : MarshalByRefObject, IServerAdminInterface
    {
        ...
        public bool StopServerControlled(bool AForceAutomaticClosing)
        {
            ...
            if (ClientsConnected > 0)
            {
                ...
                TLogging.Log("  " + String.Format(Catalog.GetString("CONTROLLED SHUTDOWN: Notifying all connected Clients ({0} {1}). Please wait..."), ClientsConnected, ClientsConnected > 1 ? "Clients" : "Client"));
...
CheckAllClientsDisconnected:                
                    ...
                    && (Retries < MAX_RETRIES))
                {
                    ...
                    if (Retries % 4 == 1)
                    {
                        TLogging.Log("    " + String.Format(Catalog.GetString("CONTROLLED SHUTDOWN: There {2} still {0} {1} connected. Waiting for {3} to disconnect (Waiting {4} more seconds)..."), ClientsConnected, ClientsConnected > 1 ? "Clients" : "Client",
                                                            ClientsConnected > 1 ? "are" : "is", ClientsConnected > 1 ? "them" : "it", ((SLEEP_TIME_PER_RETRY * MAX_RETRIES) - (SLEEP_TIME_PER_RETRY * Retries)) / 1000));
                    }
                ...
                if (Retries == MAX_RETRIES) 
                {
                    ...
                    TLogging.Log("  " + String.Format(Catalog.GetString("CONTROLLED SHUTDOWN: {0} did not respond to the disconnect request. Enter FORCE to force closing of the {1} and shutdown the Server, or anything else to leave this command."), ClientsConnected == 1 ? "One Client" : ClientsConnected + " Clients", ClientsConnected > 1 ? "Clients" : "Client"));
                    ...
                    if (Console.ReadLine() == "FORCE")
                    {
                        ...
                        {
                            // Yes there is still at least one Client connected
                            TLogging.Log("    " + String.Format(Catalog.GetString("CONTROLLED SHUTDOWN: Forcing all connected Clients ({0} {1}) to close. Please wait..."), 
                            ...
                            {
                                TLogging.Log("    " + String.Format(Catalog.GetString("CONTROLLED SHUTDOWN: Waiting {0} seconds so the Server can be sure that all Clients have got the message that they need to close... ({1} more seconds to wait)"), TSrvSetting.ClientKeepAliveCheckIntervalInSeconds / 1000, 
                                                                    (TSrvSetting.ClientKeepAliveCheckIntervalInSeconds - ((TSrvSetting.ClientKeepAliveCheckIntervalInSeconds / 4) * (Counter - 1))) / 1000));
                        ...
                        else
                        {
                            ...
                            TLogging.Log("  " + Catalog.GetString("CONTROLLED SHUTDOWN: All Clients have disconnected in the meantine, proceeding with shutdown immediately."));
                        }
                    }
                    else
                    {
