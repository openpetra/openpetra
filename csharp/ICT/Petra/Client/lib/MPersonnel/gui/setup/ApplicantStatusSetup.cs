// auto generated with nant generateWinforms from ApplicantStatusSetup.yaml and template windowMaintainCachableTable
//
...
namespace Ict.Petra.Client.MPersonnel.Gui.Setup
{
  ...
  /// auto generated: Maintain Applicant Statuses
  public partial class TFrmApplicantStatusSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    private TFrmPetraEditUtils FPetraUtilsObject;

    private class FMainDS
    {
        public static PtApplicantStatusTable PtApplicantStatus;
    }
    /// constructor
    public TFrmApplicantStatusSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.lblDetailCode.Text = Catalog.GetString("Applicant Status:");
      ...
      this.tbbNew.Text = Catalog.GetString("New Applicant Status");
      ...
      this.Text = Catalog.GetString("Maintain Applicant Statuses");
      #endregion
      ...
      FPetraUtilsObject = new TFrmPetraEditUtils(AParentFormHandle, this, stbMain);
            FPetraUtilsObject.SetStatusBarText(txtDetailCode, Catalog.GetString("This code indicates the status of an applicant."));
      FPetraUtilsObject.SetStatusBarText(txtDetailDescription, Catalog.GetString("This describes the applicant status code."));
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PtApplicantStatus = new PtApplicantStatusTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("ApplicantStatusList", String.Empty, null, out DataTableType);
      FMainDS.PtApplicantStatus.Merge(CacheDT);

      grdDetails.Columns.Clear();
      grdDetails.AddTextColumn("Applicant Status", FMainDS.PtApplicantStatus.ColumnCode);
      grdDetails.AddTextColumn("Description", FMainDS.PtApplicantStatus.ColumnDescription);
      grdDetails.AddCheckBoxColumn("Unassignable?", FMainDS.PtApplicantStatus.ColumnUnassignableFlag);
      grdDetails.AddDateColumn("Unassignable Date", FMainDS.PtApplicantStatus.ColumnUnassignableDate);
      grdDetails.AddCheckBoxColumn("Deletable", FMainDS.PtApplicantStatus.ColumnDeletableFlag);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.PtApplicantStatus.DefaultView;
      myDataView.AllowNew = false;
      grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(myDataView);

      // Ensure that the Details Panel is disabled if there are no records
      if (FMainDS.PtApplicantStatus.Rows.Count == 0)
      {
        ShowDetails(null);
      }
    ...
    /// automatically generated, create a new record of PtApplicantStatus and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPtApplicantStatus()
    {
        PtApplicantStatusRow NewRow = FMainDS.PtApplicantStatus.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PtApplicantStatus.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PtApplicantStatus.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PtApplicantStatus.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PtApplicantStatus.PrimaryKey)
            {
                string value1 = FMainDS.PtApplicantStatus.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PtApplicantStatusRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PtApplicantStatusRow)SelectedGridRow[0].Row;
        }
    ...
    private void ShowDetails(PtApplicantStatusRow ARow)
    {
    ...
    private PtApplicantStatusRow FPreviouslySelectedDetailRow = null;
    ...
    private void GetDetailsFromControls(PtApplicantStatusRow ARow)
    {
        if (ARow != null)
        {
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PtApplicantStatus.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PtApplicantStatus.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("ApplicantStatusList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PtApplicantStatus.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PtApplicantStatus.Merge(SubmitDT, false);
