  ...
  public partial class TFrmDocumentTypeCategorySetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    public TFrmDocumentTypeCategorySetup(IntPtr AParentFormHandle) : base()
    {
      ...
      FPetraUtilsObject = new TFrmPetraEditUtils(AParentFormHandle, this, stbMain);
            FPetraUtilsObject.SetStatusBarText(txtDetailCode, Catalog.GetString("This code indicates the category of a document type."));
      FPetraUtilsObject.SetStatusBarText(txtDetailDescription, Catalog.GetString("This describes the document type category."));
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PmDocumentCategory = new PmDocumentCategoryTable();
      ...
      grdDetails.AddCheckBoxColumn("Deletable?", FMainDS.PmDocumentCategory.ColumnDeletableFlag);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.PmDocumentCategory.DefaultView;
      myDataView.AllowNew = false;
      grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(myDataView);

      // Ensure that the Details Panel is disabled if there are no records
      if (FMainDS.PmDocumentCategory.Rows.Count == 0)
      {
        ShowDetails(null);
      }
    ...
    /// automatically generated, create a new record of PmDocumentCategory and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPmDocumentCategory()
    {
        PmDocumentCategoryRow NewRow = FMainDS.PmDocumentCategory.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PmDocumentCategory.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PmDocumentCategory.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PmDocumentCategory.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PmDocumentCategory.PrimaryKey)
            {
                string value1 = FMainDS.PmDocumentCategory.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PmDocumentCategoryRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PmDocumentCategoryRow)SelectedGridRow[0].Row;
        }
    ...
    private void ShowDetails(PmDocumentCategoryRow ARow)
    {
    ...
    private PmDocumentCategoryRow FPreviouslySelectedDetailRow = null;
    ...
    private void GetDetailsFromControls(PmDocumentCategoryRow ARow)
    {
        if (ARow != null)
        {
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PmDocumentCategory.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PmDocumentCategory.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("DocumentTypeCategoryList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PmDocumentCategory.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PmDocumentCategory.Merge(SubmitDT, false);
