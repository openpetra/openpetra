// auto generated with nant generateWinforms from PositionSetup.yaml and template windowMaintainCachableTable
//
...
namespace Ict.Petra.Client.MPersonnel.Gui.Setup
{
  ...
  /// auto generated: Maintain Positions
  public partial class TFrmPositionSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    private TFrmPetraEditUtils FPetraUtilsObject;

    private class FMainDS
    {
        public static PtPositionTable PtPosition;
    }
    /// constructor
    public TFrmPositionSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.tbbNew.Text = Catalog.GetString("New Position");
      ...
      this.Text = Catalog.GetString("Maintain Positions");
      #endregion
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PtPosition = new PtPositionTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("PositionList", String.Empty, null, out DataTableType);
      FMainDS.PtPosition.Merge(CacheDT);
      ...
      grdDetails.AddCheckBoxColumn("Deletable", FMainDS.PtPosition.ColumnDeletableFlag);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.PtPosition.DefaultView;
      myDataView.AllowNew = false;
      grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(myDataView);

      // Ensure that the Details Panel is disabled if there are no records
      if (FMainDS.PtPosition.Rows.Count == 0)
      {
        ShowDetails(null);
      }
    ...
    /// automatically generated, create a new record of PtPosition and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPtPosition()
    {
        PtPositionRow NewRow = FMainDS.PtPosition.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PtPosition.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PtPosition.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PtPosition.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PtPosition.PrimaryKey)
            {
                string value1 = FMainDS.PtPosition.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PtPositionRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PtPositionRow)SelectedGridRow[0].Row;
        }
    ...
    private void ShowDetails(PtPositionRow ARow)
    {
        ...
        else
        {
            ...
            {
                txtDetailPositionDescr.Text = ARow.PositionDescr;
            }
    ...
    private PtPositionRow FPreviouslySelectedDetailRow = null;
    ...
    private void GetDetailsFromControls(PtPositionRow ARow)
    {
        if (ARow != null)
        {
            ...
            {
                ARow.PositionDescr = txtDetailPositionDescr.Text;
            }
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PtPosition.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PtPosition.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("PositionList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PtPosition.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PtPosition.Merge(SubmitDT, false);
