...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  /// auto generated: Maintain Denomination Codes
  public partial class TFrmDenominationCodeSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    public TFrmDenominationCodeSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.lblDetailDenominationCode.Text = Catalog.GetString("Denomination Code:");
      this.lblDetailDenominationName.Text = Catalog.GetString("Denomination Name:");
      this.lblDetailValidDenomination.Text = Catalog.GetString("Assignable:");
      this.lblDetailDeletable.Text = Catalog.GetString("Deletable:");
      this.tbbSave.ToolTipText = Catalog.GetString("Saves changed data");
      this.tbbSave.Text = Catalog.GetString("&Save");
      this.tbbNew.Text = Catalog.GetString("New Denomination Code");
      ...
      this.Text = Catalog.GetString("Maintain Denomination Codes");
      #endregion

      this.txtDetailDenominationCode.Font = TAppSettingsManager.GetDefaultBoldFont();
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PDenomination = new PDenominationTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("DenominationList", String.Empty, null, out DataTableType);
      FMainDS.PDenomination.Merge(CacheDT);

      grdDetails.Columns.Clear();
      grdDetails.AddTextColumn("Denomination Code", FMainDS.PDenomination.ColumnDenominationCode);
      grdDetails.AddTextColumn("Denomination Name", FMainDS.PDenomination.ColumnDenominationName);
      grdDetails.AddCheckBoxColumn("Assignable", FMainDS.PDenomination.ColumnValidDenomination);
      grdDetails.AddCheckBoxColumn("Deletable", FMainDS.PDenomination.ColumnDeletable);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.PDenomination.DefaultView;
      myDataView.AllowNew = false;
      grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(myDataView);

      // Ensure that the Details Panel is disabled if there are no records
      if (FMainDS.PDenomination.Rows.Count == 0)
      {
        ShowDetails(null);
      }
    ...
    /// automatically generated, create a new record of PDenomination and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPDenomination()
    {
        PDenominationRow NewRow = FMainDS.PDenomination.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PDenomination.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PDenomination.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PDenomination.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PDenomination.PrimaryKey)
            {
                string value1 = FMainDS.PDenomination.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PDenominationRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PDenominationRow)SelectedGridRow[0].Row;
        }
    ...
    {
        txtDetailDenominationCode.ReadOnly = AReadOnly;
    }

    private void ShowDetails(PDenominationRow ARow)
    {
        ...
        else
        {
            ...
            txtDetailDenominationCode.Text = ARow.DenominationCode;
            txtDetailDenominationCode.ReadOnly = (ARow.RowState != DataRowState.Added);
            txtDetailDenominationName.Text = ARow.DenominationName;
            chkDetailValidDenomination.Checked = ARow.ValidDenomination;
    ...
    private PDenominationRow FPreviouslySelectedDetailRow = null;
    ...
    private void GetDetailsFromControls(PDenominationRow ARow)
    {
        if (ARow != null)
        {
            ARow.DenominationCode = txtDetailDenominationCode.Text;
            ARow.DenominationName = txtDetailDenominationName.Text;
            ARow.ValidDenomination = chkDetailValidDenomination.Checked;
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PDenomination.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PDenomination.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("DenominationList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PDenomination.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PDenomination.Merge(SubmitDT, false);
