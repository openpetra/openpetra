...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  public partial class TFrmRelationCategorySetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PRelationCategory = new PRelationCategoryTable();
      ...
      DataView myDataView = FMainDS.PRelationCategory.DefaultView;
    ...
    /// automatically generated, create a new record of PRelationCategory and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPRelationCategory()
    {
        PRelationCategoryRow NewRow = FMainDS.PRelationCategory.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PRelationCategory.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PRelationCategory.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PRelationCategory.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PRelationCategory.PrimaryKey)
            {
                string value1 = FMainDS.PRelationCategory.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PRelationCategoryRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PRelationCategoryRow)SelectedGridRow[0].Row;
        }
    ...
    private void GetDetailsFromControls(PRelationCategoryRow ARow)
    {
        if (ARow != null)
        {
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PRelationCategory.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PRelationCategory.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("RelationCategoryList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PRelationCategory.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PRelationCategory.Merge(SubmitDT, false);
