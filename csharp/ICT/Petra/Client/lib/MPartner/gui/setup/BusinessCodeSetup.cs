...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  /// auto generated: Maintain Business Codes
  public partial class TFrmBusinessCodeSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    public TFrmBusinessCodeSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.lblDetailBusinessCode.Text = Catalog.GetString("Business Code:");
      this.lblDetailBusinessDescription.Text = Catalog.GetString("Business Description:");
      this.lblDetailDeletable.Text = Catalog.GetString("Deletable:");
      this.tbbSave.ToolTipText = Catalog.GetString("Saves changed data");
      this.tbbSave.Text = Catalog.GetString("&Save");
      this.tbbNew.Text = Catalog.GetString("New Business Code");
      ...
      this.Text = Catalog.GetString("Maintain Business Codes");
      #endregion

      this.txtDetailBusinessCode.Font = TAppSettingsManager.GetDefaultBoldFont();
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PBusiness = new PBusinessTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("BusinessCodeList", String.Empty, null, out DataTableType);
      FMainDS.PBusiness.Merge(CacheDT);

      grdDetails.Columns.Clear();
      grdDetails.AddTextColumn("Business Code", FMainDS.PBusiness.ColumnBusinessCode);
      grdDetails.AddTextColumn("Description", FMainDS.PBusiness.ColumnBusinessDescription);
      grdDetails.AddCheckBoxColumn("Deletable", FMainDS.PBusiness.ColumnDeletable);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.PBusiness.DefaultView;
    ...
    /// automatically generated, create a new record of PBusiness and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPBusiness()
    {
        PBusinessRow NewRow = FMainDS.PBusiness.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PBusiness.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PBusiness.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PBusiness.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PBusiness.PrimaryKey)
            {
                string value1 = FMainDS.PBusiness.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PBusinessRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PBusinessRow)SelectedGridRow[0].Row;
        }
    ...
    private void GetDetailsFromControls(PBusinessRow ARow)
    {
        if (ARow != null)
        {
            ARow.BusinessCode = txtDetailBusinessCode.Text;
            ARow.BusinessDescription = txtDetailBusinessDescription.Text;
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PBusiness.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PBusiness.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("BusinessCodeList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PBusiness.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PBusiness.Merge(SubmitDT, false);
