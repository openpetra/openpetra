...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  public partial class TFrmPublicationSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    public TFrmPublicationSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.lblDetailPublicationCode.Text = Catalog.GetString("Publication Code:");
      this.lblDetailPublicationDescription.Text = Catalog.GetString("Publication Description:");
      this.lblDetailPublicationLabelCode.Text = Catalog.GetString("Publication Label Code:");
      this.lblDetailNumberOfIssues.Text = Catalog.GetString("Number Of Issues:");
      this.lblDetailNumberOfReminders.Text = Catalog.GetString("Number Of Reminders:");
      this.lblDetailFrequencyCode.Text = Catalog.GetString("Frequency Code:");
      this.lblDetailValidPublication.Text = Catalog.GetString("Valid Publication:");
      this.tbbSave.ToolTipText = Catalog.GetString("Saves changed data");
      this.tbbSave.Text = Catalog.GetString("&Save");
      this.tbbNew.Text = Catalog.GetString("New Publication Code");
      ...
      this.txtDetailPublicationDescription.Font = TAppSettingsManager.GetDefaultBoldFont();
      this.txtDetailPublicationLabelCode.Font = TAppSettingsManager.GetDefaultBoldFont();
      this.txtDetailNumberOfIssues.Font = TAppSettingsManager.GetDefaultBoldFont();
      ...
      FPetraUtilsObject.SetStatusBarText(txtDetailNumberOfReminders, Catalog.GetString("Enter the number of issues and reminders to send out"));
      FPetraUtilsObject.SetStatusBarText(cmbDetailFrequencyCode, Catalog.GetString("Select how often this publication will be produced"));
      cmbDetailFrequencyCode.InitialiseUserControl();
      FPetraUtilsObject.SetStatusBarText(chkDetailValidPublication, Catalog.GetString("Select if publication can be selected by users"));
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PPublication = new PPublicationTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("PublicationList", String.Empty, null, out DataTableType);
      FMainDS.PPublication.Merge(CacheDT);

      grdDetails.Columns.Clear();
      grdDetails.AddTextColumn("Publication Code", FMainDS.PPublication.ColumnPublicationCode);
      grdDetails.AddTextColumn("Description", FMainDS.PPublication.ColumnPublicationDescription);
      grdDetails.AddTextColumn("Frequency", FMainDS.PPublication.ColumnFrequencyCode);
      grdDetails.AddCheckBoxColumn("Valid Publication", FMainDS.PPublication.ColumnValidPublication);
      grdDetails.AddTextColumn("Publication Label Code", FMainDS.PPublication.ColumnPublicationLabelCode);
    ...
    /// automatically generated, create a new record of PPublication and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPPublication()
    {
        PPublicationRow NewRow = FMainDS.PPublication.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PPublication.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PPublication.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PPublication.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PPublication.PrimaryKey)
            {
                string value1 = FMainDS.PPublication.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PPublicationRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PPublicationRow)SelectedGridRow[0].Row;
        }
    ...
    {
        txtDetailPublicationCode.ReadOnly = AReadOnly;
    }
    ...
    private void GetDetailsFromControls(PPublicationRow ARow)
    {
        if (ARow != null)
        {
            ARow.PublicationCode = txtDetailPublicationCode.Text;
            if (txtDetailPublicationDescription.Text.Length == 0)
            {
                ARow.SetPublicationDescriptionNull();
            }
            else
            {
                ARow.PublicationDescription = txtDetailPublicationDescription.Text;
            }
            if (txtDetailPublicationLabelCode.Text.Length == 0)
            {
                ARow.SetPublicationLabelCodeNull();
            }
            else
            {
                ARow.PublicationLabelCode = txtDetailPublicationLabelCode.Text;
            }
            if (txtDetailNumberOfIssues.Text.Length == 0)
            {
                ARow.SetNumberOfIssuesNull();
            }
            else
            {
                ARow.NumberOfIssues = Convert.ToInt32(txtDetailNumberOfIssues.Text);
            }
            if (txtDetailNumberOfReminders.Text.Length == 0)
            {
                ARow.SetNumberOfRemindersNull();
            }
            else
            {
                ARow.NumberOfReminders = Convert.ToInt32(txtDetailNumberOfReminders.Text);
            }
            ARow.FrequencyCode = cmbDetailFrequencyCode.GetSelectedString();
            ARow.ValidPublication = chkDetailValidPublication.Checked;
        }
    }
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PPublication.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PPublication.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("PublicationList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PPublication.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PPublication.Merge(SubmitDT, false);
