// auto generated with nant generateWinforms from AcquisitionCodeSetup.yaml and template windowMaintainCachableTable
//
...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  public partial class TFrmAcquisitionCodeSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    {
        public static PAcquisitionTable PAcquisition;
    }
    /// constructor
    public TFrmAcquisitionCodeSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.txtDetailAcquisitionDescription.Font = TAppSettingsManager.GetDefaultBoldFont();
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PAcquisition = new PAcquisitionTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("AcquisitionCodeList", String.Empty, null, out DataTableType);
      FMainDS.PAcquisition.Merge(CacheDT);
      ...
      DataView myDataView = FMainDS.PAcquisition.DefaultView;
    ...
    /// automatically generated, create a new record of PAcquisition and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPAcquisition()
    {
        PAcquisitionRow NewRow = FMainDS.PAcquisition.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PAcquisition.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PAcquisition.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PAcquisition.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PAcquisition.PrimaryKey)
            {
                string value1 = FMainDS.PAcquisition.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PAcquisitionRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PAcquisitionRow)SelectedGridRow[0].Row;
        }
    ...
    {
        txtDetailAcquisitionCode.ReadOnly = AReadOnly;
    }
    ...
    private void GetDetailsFromControls(PAcquisitionRow ARow)
    {
        if (ARow != null)
        {
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PAcquisition.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PAcquisition.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("AcquisitionCodeList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PAcquisition.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PAcquisition.Merge(SubmitDT, false);
