// auto generated with nant generateWinforms from OccupationCodeSetup.yaml and template windowMaintainCachableTable
//
...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  public partial class TFrmOccupationCodeSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    {
        public static POccupationTable POccupation;
    }
    /// constructor
    public TFrmOccupationCodeSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.lblDetailOccupationCode.Text = Catalog.GetString("Occupation Code:");
      this.lblDetailOccupationDescription.Text = Catalog.GetString("Occupation Description:");
      this.lblDetailValidOccupation.Text = Catalog.GetString("Assignable:");
      this.lblDetailDeletable.Text = Catalog.GetString("Deletable:");
      this.tbbSave.ToolTipText = Catalog.GetString("Saves changed data");
      this.tbbSave.Text = Catalog.GetString("&Save");
      this.tbbNew.Text = Catalog.GetString("New Occupation Code");
      ...
      this.txtDetailOccupationDescription.Font = TAppSettingsManager.GetDefaultBoldFont();

      FPetraUtilsObject = new TFrmPetraEditUtils(AParentFormHandle, this, stbMain);
            FPetraUtilsObject.SetStatusBarText(txtDetailOccupationCode, Catalog.GetString("Enter an approved occupation code"));
      FPetraUtilsObject.SetStatusBarText(txtDetailOccupationDescription, Catalog.GetString("Enter a full description or name"));
      FPetraUtilsObject.SetStatusBarText(chkDetailValidOccupation, Catalog.GetString("Select this to make this available to selection lists"));
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.POccupation = new POccupationTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("OccupationList", String.Empty, null, out DataTableType);
      FMainDS.POccupation.Merge(CacheDT);

      grdDetails.Columns.Clear();
      grdDetails.AddTextColumn("Occupation Code", FMainDS.POccupation.ColumnOccupationCode);
      grdDetails.AddTextColumn("Occupation", FMainDS.POccupation.ColumnOccupationDescription);
      grdDetails.AddCheckBoxColumn("Assignable", FMainDS.POccupation.ColumnValidOccupation);
      grdDetails.AddCheckBoxColumn("Deletable", FMainDS.POccupation.ColumnDeletable);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.POccupation.DefaultView;
    ...
    /// automatically generated, create a new record of POccupation and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPOccupation()
    {
        POccupationRow NewRow = FMainDS.POccupation.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.POccupation.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.POccupation.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.POccupation.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.POccupation.PrimaryKey)
            {
                string value1 = FMainDS.POccupation.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private POccupationRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (POccupationRow)SelectedGridRow[0].Row;
        }
    ...
    {
        txtDetailOccupationCode.ReadOnly = AReadOnly;
    }
    ...
    private void GetDetailsFromControls(POccupationRow ARow)
    {
        if (ARow != null)
        {
            ARow.OccupationCode = txtDetailOccupationCode.Text;
            ARow.OccupationDescription = txtDetailOccupationDescription.Text;
            ARow.ValidOccupation = chkDetailValidOccupation.Checked;
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.POccupation.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.POccupation.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("OccupationList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.POccupation.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.POccupation.Merge(SubmitDT, false);
