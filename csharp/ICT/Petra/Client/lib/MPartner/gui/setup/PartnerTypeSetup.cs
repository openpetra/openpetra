// auto generated with nant generateWinforms from PartnerTypeSetup.yaml and template windowMaintainCachableTable
//
...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  public partial class TFrmPartnerTypeSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    public TFrmPartnerTypeSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.txtDetailTypeDescription.Font = TAppSettingsManager.GetDefaultBoldFont();

      FPetraUtilsObject = new TFrmPetraEditUtils(AParentFormHandle, this, stbMain);
            FPetraUtilsObject.SetStatusBarText(txtDetailTypeCode, Catalog.GetString("Enter a type code"));
    ...
    /// automatically generated, create a new record of PType and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPType()
    {
        PTypeRow NewRow = FMainDS.PType.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PType.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PType.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PType.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PType.PrimaryKey)
            {
                string value1 = FMainDS.PType.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PTypeRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PTypeRow)SelectedGridRow[0].Row;
        }
    ...
    {
        txtDetailTypeCode.ReadOnly = AReadOnly;
    }
    ...
    private void GetDetailsFromControls(PTypeRow ARow)
    {
        if (ARow != null)
        {
            ARow.TypeCode = txtDetailTypeCode.Text;
            ARow.TypeDescription = txtDetailTypeDescription.Text;
            ARow.ValidType = chkDetailValidType.Checked;
            ARow.TypeDeletable = chkDetailTypeDeletable.Checked;
        }
    }
