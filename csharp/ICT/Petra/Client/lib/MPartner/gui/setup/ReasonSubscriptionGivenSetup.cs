...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  /// auto generated: Maintain Reason Subscription Given List
  public partial class TFrmReasonSubscriptionGivenSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    public TFrmReasonSubscriptionGivenSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.lblDetailCode.Text = Catalog.GetString("Reason Given Code:");
      this.lblDetailDescription.Text = Catalog.GetString("Description:");
      this.tbbSave.ToolTipText = Catalog.GetString("Saves changed data");
      this.tbbSave.Text = Catalog.GetString("&Save");
      this.tbbNew.Text = Catalog.GetString("New Reason Given Code");
      ...
      this.Text = Catalog.GetString("Maintain Reason Subscription Given List");
      #endregion
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PReasonSubscriptionGiven = new PReasonSubscriptionGivenTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("ReasonSubscriptionGivenList", String.Empty, null, out DataTableType);
      FMainDS.PReasonSubscriptionGiven.Merge(CacheDT);

      grdDetails.Columns.Clear();
      grdDetails.AddTextColumn("Reason Given Code", FMainDS.PReasonSubscriptionGiven.ColumnCode);
      grdDetails.AddTextColumn("Description", FMainDS.PReasonSubscriptionGiven.ColumnDescription);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.PReasonSubscriptionGiven.DefaultView;
    ...
    /// automatically generated, create a new record of PReasonSubscriptionGiven and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPReasonSubscriptionGiven()
    {
        PReasonSubscriptionGivenRow NewRow = FMainDS.PReasonSubscriptionGiven.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PReasonSubscriptionGiven.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PReasonSubscriptionGiven.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PReasonSubscriptionGiven.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PReasonSubscriptionGiven.PrimaryKey)
            {
                string value1 = FMainDS.PReasonSubscriptionGiven.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PReasonSubscriptionGivenRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PReasonSubscriptionGivenRow)SelectedGridRow[0].Row;
        }
    ...
    private void GetDetailsFromControls(PReasonSubscriptionGivenRow ARow)
    {
        if (ARow != null)
        {
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PReasonSubscriptionGiven.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PReasonSubscriptionGiven.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("ReasonSubscriptionGivenList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PReasonSubscriptionGiven.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PReasonSubscriptionGiven.Merge(SubmitDT, false);
