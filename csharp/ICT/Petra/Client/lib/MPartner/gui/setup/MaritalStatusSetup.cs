// auto generated with nant generateWinforms from MaritalStatusSetup.yaml and template windowMaintainCachableTable
//
...
namespace Ict.Petra.Client.MPartner.Gui.Setup
{
  ...
  public partial class TFrmMaritalStatusSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    {
        public static PtMaritalStatusTable PtMaritalStatus;
    }
    /// constructor
    public TFrmMaritalStatusSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.lblDetailAssignableFlag.Text = Catalog.GetString("Assignable:");
      this.lblDetailAssignableDate.Text = Catalog.GetString("Unassignable Date:");
      ...
      FPetraUtilsObject = new TFrmPetraEditUtils(AParentFormHandle, this, stbMain);
            FPetraUtilsObject.SetStatusBarText(txtDetailCode, Catalog.GetString("This code indicates the different marital statuses."));
      FPetraUtilsObject.SetStatusBarText(txtDetailDescription, Catalog.GetString("This describes the marital statuses."));
      FPetraUtilsObject.SetStatusBarText(chkDetailAssignableFlag, Catalog.GetString("Indicates if this code can still be assigned?"));
      FPetraUtilsObject.SetStatusBarText(dtpDetailAssignableDate, Catalog.GetString("Date from which this code was made unassignable."));
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PtMaritalStatus = new PtMaritalStatusTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("MaritalStatusList", String.Empty, null, out DataTableType);
      FMainDS.PtMaritalStatus.Merge(CacheDT);

      grdDetails.Columns.Clear();
      grdDetails.AddTextColumn("Marital Status Code", FMainDS.PtMaritalStatus.ColumnCode);
      grdDetails.AddTextColumn("Description", FMainDS.PtMaritalStatus.ColumnDescription);
      grdDetails.AddCheckBoxColumn("Assignable", FMainDS.PtMaritalStatus.ColumnAssignableFlag);
      grdDetails.AddDateColumn("Unassignable Date", FMainDS.PtMaritalStatus.ColumnAssignableDate);
      grdDetails.AddCheckBoxColumn("Deletable", FMainDS.PtMaritalStatus.ColumnDeletableFlag);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.PtMaritalStatus.DefaultView;
    ...
    /// automatically generated, create a new record of PtMaritalStatus and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPtMaritalStatus()
    {
        PtMaritalStatusRow NewRow = FMainDS.PtMaritalStatus.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PtMaritalStatus.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PtMaritalStatus.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PtMaritalStatus.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PtMaritalStatus.PrimaryKey)
            {
                string value1 = FMainDS.PtMaritalStatus.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PtMaritalStatusRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PtMaritalStatusRow)SelectedGridRow[0].Row;
        }
    ...
    private void GetDetailsFromControls(PtMaritalStatusRow ARow)
    {
        if (ARow != null)
        {
            ARow.Code = txtDetailCode.Text;
            ARow.Description = txtDetailDescription.Text;
            ARow.AssignableFlag = chkDetailAssignableFlag.Checked;
            if (dtpDetailAssignableDate.Date == null)
            {
                ARow.SetAssignableDateNull();
            }
            else
            {
                ARow.AssignableDate = dtpDetailAssignableDate.Date.Value;
            }
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PtMaritalStatus.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PtMaritalStatus.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("MaritalStatusList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PtMaritalStatus.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PtMaritalStatus.Merge(SubmitDT, false);
