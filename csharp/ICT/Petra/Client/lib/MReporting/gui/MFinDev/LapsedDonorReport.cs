...
namespace Ict.Petra.Client.MReporting.Gui.MFinDev
{
  ...
  /// </summary>
  public partial class TFrmLapsedDonorReport: System.Windows.Forms.Form, IFrmReporting
  {
    ...
    /// constructor
    /// </summary>
    public TFrmLapsedDonorReport(IntPtr AParentFormHandle) : base()
    {
      //
      ...
      this.lblToleranceDays.Text = Catalog.GetString("+/- Days:");
      this.lblFrequency.Text = Catalog.GetString("Frequency:");
      this.lblMinimumAmount.Text = Catalog.GetString("Minimum Amount:");
      this.lblIgnoreBetween.Text = Catalog.GetString("Ignore if other gifts between:");
      this.grpDateSelection.Text = Catalog.GetString("Selection");
      this.lblCurrency.Text = Catalog.GetString("Currency:");
      this.lblMotivationGroup.Text = Catalog.GetString("Motivation Group:");
      this.lblMotivationDetail.Text = Catalog.GetString("Motivation Detail:");
      this.txtRecipient.ButtonText = Catalog.GetString("Find");
      this.lblRecipient.Text = Catalog.GetString("Recipient:");
      this.rbtAllPartners.Text = Catalog.GetString("All Partners");
      this.rbtExtract.Text = Catalog.GetString("Extract");
      this.txtExtract.ButtonText = Catalog.GetString("Find");
      this.grpOtherSelection.Text = Catalog.GetString("Other");
      this.rbtSortByPartnerName.Text = Catalog.GetString("By Partner Name");
      this.rbtSortByDonorKey.Text = Catalog.GetString("By Partner Key");
      this.rgrSorting.Text = Catalog.GetString("Sort");
      ...
      this.Text = Catalog.GetString("Lapsed Donor Report");
      #endregion

      this.txtMotivationGroup.Font = TAppSettingsManager.GetDefaultBoldFont();
      this.txtMotivationDetail.Font = TAppSettingsManager.GetDefaultBoldFont();

      FPetraUtilsObject = new TFrmPetraReportingUtils(AParentFormHandle, this, stbMain);

      FPetraUtilsObject.FXMLFiles = "FinancialDevelopment\\\\lapseddonorreport.xml,common.xml";
      FPetraUtilsObject.FReportName = "LapsedDonorReport";
      FPetraUtilsObject.FCurrentReport = "LapsedDonorReport";
...
#region Parameter/Settings Handling
    ...
    */
    public void ReadControls(TRptCalculator ACalc, TReportActionEnum AReportAction)
    {
      ...
      ACalc.AddParameter("param_days_tolerance", this.txtToleranceDays.Text);
      if (this.cmbFrequency.SelectedItem != null)
      {
          ACalc.AddParameter("param_frequency", this.cmbFrequency.SelectedItem.ToString());
      }
      else
      {
          ACalc.AddParameter("param_frequency", "");
      }
      ACalc.AddParameter("param_minimum_amount", this.txtMinimumAmount.Text);
      ACalc.AddParameter("param_ignore_between", this.chkIgnoreBetween.Checked);
      ...
      ACalc.AddParameter("param_motivation_group", this.txtMotivationGroup.Text);
      ACalc.AddParameter("param_motivation_detail", this.txtMotivationDetail.Text);
      if (rbtSortByPartnerName.Checked)
      {
          ACalc.AddParameter("param_sort_by", "Donor_Name");
      }
      if (rbtSortByDonorKey.Checked)
      {
          ACalc.AddParameter("param_sort_by", "Donor_Key");
      }
    ...
    */
    public void SetControls(TParameterList AParameters)
    {
      ...
      txtToleranceDays.NumberValueInt = AParameters.Get("param_days_tolerance").ToInt32();
      cmbFrequency.SelectedValue = AParameters.Get("param_frequency").ToString();
      txtMinimumAmount.NumberValueInt = AParameters.Get("param_minimum_amount").ToInt32();
      chkIgnoreBetween.Checked = AParameters.Get("param_ignore_between").ToBool();
      cmbCurrency.SelectedValue = AParameters.Get("param_currency").ToString();
      txtMotivationGroup.Text = AParameters.Get("param_motivation_group").ToString();
      txtMotivationDetail.Text = AParameters.Get("param_motivation_detail").ToString();
      rbtSortByPartnerName.Checked = AParameters.Get("param_sort_by").ToString() == "Donor_Name";
      rbtSortByDonorKey.Checked = AParameters.Get("param_sort_by").ToString() == "Donor_Key";
...
#region Column Functions and Calculations
    ...
    */
    public void SetAvailableFunctions()
    {
      ...
      FPetraUtilsObject.AddAvailableFunction(new TPartnerColumnFunction("Donor Name", 4.0));
      ...
      FPetraUtilsObject.AddAvailableFunction(new TPartnerColumnFunction("Gift this year", 2.0));
      FPetraUtilsObject.AddAvailableFunction(new TPartnerColumnFunction("Gift year - 1", 2.0));
      FPetraUtilsObject.AddAvailableFunction(new TPartnerColumnFunction("Gift year - 2", 2.0));
