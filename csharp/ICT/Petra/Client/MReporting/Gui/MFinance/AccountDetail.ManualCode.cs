    ...
    public partial class TFrmAccountDetail
    {
        ...
        private void LoadReportData(TRptCalculator ACalc)
        {
            ...
            if (pm.Get("param_period").ToBool() == true)
            {
                ...
                AccountingPeriodTbl.DefaultView.RowFilter = LedgerFilter + " AND a_accounting_period_number_i=" + pm.Get("param_start_period_i").ToInt32();
                pm.Add("param_start_date", Convert.ToDateTime(AccountingPeriodTbl.DefaultView[0].Row["a_period_start_date_d"]));

                AccountingPeriodTbl.DefaultView.RowFilter = LedgerFilter + " AND a_accounting_period_number_i=" + pm.Get("param_end_period_i").ToInt32();
            ...
            {
                Csv = StringHelper.AddCSV(Csv, "ATransaction/"
            ...
            if (ReportDs.ATransaction.Rows.Count > 0)
            {
                ...
                Csv = "";
                Csv = StringHelper.AddCSV(Csv, "ATransAnalAttrib/*/a_trans_anal_attrib/" + LedgerFilter + " AND a_batch_number_i >= " + FirstBatch + " AND a_batch_number_i <= " + LastBatch);
