// auto generated with nant generateWinforms from SetupAdminGrantsPayable.yaml and template windowMaintainTable
//
...
namespace Ict.Petra.Client.MFinance.Gui.Setup
{
  ...
  /// auto generated: Maintain Payable Administration Grants
  public partial class TFrmSetupAdminGrantsPayable: System.Windows.Forms.Form, IFrmPetraEdit
  {
    private TFrmPetraEditUtils FPetraUtilsObject;

    private class FMainDS
    {
        public static AFeesPayableTable AFeesPayable;
    }

    /// constructor
    public TFrmSetupAdminGrantsPayable(IntPtr AParentFormHandle) : base()
    {
      ...
      this.tbbNew.Text = Catalog.GetString("New Payable Admin. Grant");
      ...
      this.Text = Catalog.GetString("Maintain Payable Administration Grants");
      #endregion
      ...
      this.txtReceivingFund.Font = TAppSettingsManager.GetDefaultBoldFont();

      FPetraUtilsObject = new TFrmPetraEditUtils(AParentFormHandle, this, stbMain);
      FPetraUtilsObject.SetStatusBarText(txtDetailFeeCode, Catalog.GetString("Enter a code for this fee"));
      FPetraUtilsObject.SetStatusBarText(txtDetailFeeDescription, Catalog.GetString("Enter a description for this fee."));
      ...
      FMainDS.AFeesPayable = new AFeesPayableTable();
      ...
      grdDetails.AddTextColumn("Charge Option", FMainDS.AFeesPayable.ColumnChargeOption);
      grdDetails.AddCurrencyColumn("Charge Percentage", FMainDS.AFeesPayable.ColumnChargePercentage);
      grdDetails.AddCurrencyColumn("Charge Amount", FMainDS.AFeesPayable.ColumnChargeAmount);
      grdDetails.AddTextColumn("DR Account Code", FMainDS.AFeesPayable.ColumnDrAccountCode);
      grdDetails.AddTextColumn("Cost Centre Code", FMainDS.AFeesPayable.ColumnCostCentreCode);
      grdDetails.AddTextColumn("Account Code", FMainDS.AFeesPayable.ColumnAccountCode);
      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.AFeesPayable.DefaultView;
      myDataView.AllowNew = false;
      grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(myDataView);

      // Ensure that the Details Panel is disabled if there are no records
      if (FMainDS.AFeesPayable.Rows.Count == 0)
      {
        ShowDetails(null);
      }
    ...
    /// automatically generated, create a new record of AFeesPayable and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewAFeesPayable()
    {
        AFeesPayableRow NewRow = FMainDS.AFeesPayable.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.AFeesPayable.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.AFeesPayable.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.AFeesPayable.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.AFeesPayable.PrimaryKey)
            {
                string value1 = FMainDS.AFeesPayable.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private AFeesPayableRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (AFeesPayableRow)SelectedGridRow[0].Row;
        }
    ...
    private void ShowDetails(AFeesPayableRow ARow)
    {
    ...
    private AFeesPayableRow FPreviouslySelectedDetailRow = null;
    ...
    private void GetDetailsFromControls(AFeesPayableRow ARow)
    {
        if (ARow != null)
        {
            ARow.BeginEdit();
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.AFeesPayable.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.AFeesPayable.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TRemote.MCommon.DataReader.SaveData(AFeesPayableTable.GetTableDBName(), ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.AFeesPayable.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.AFeesPayable.Merge(SubmitDT, false);
