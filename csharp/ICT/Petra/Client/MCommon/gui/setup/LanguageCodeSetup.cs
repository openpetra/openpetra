...
namespace Ict.Petra.Client.MCommon.Gui.Setup
{
  ...
  public partial class TFrmLanguageCodeSetup: System.Windows.Forms.Form, IFrmPetraEdit
  {
    ...
    public TFrmLanguageCodeSetup(IntPtr AParentFormHandle) : base()
    {
      ...
      this.lblDetailLanguageCode.Text = Catalog.GetString("Language Code:");
      this.lblDetailLanguageDescription.Text = Catalog.GetString("Language Description:");
      this.lblDetailCongressLanguage.Text = Catalog.GetString("Congress Language:");
      this.lblDetailDeletable.Text = Catalog.GetString("Deletable:");
      this.tbbSave.ToolTipText = Catalog.GetString("Saves changed data");
      this.tbbSave.Text = Catalog.GetString("&Save");
      this.tbbNew.Text = Catalog.GetString("New Language Code");
    ...
    private void LoadDataAndFinishScreenSetup()
    {
      ...
      // Load Data
      FMainDS.PLanguage = new PLanguageTable();
      DataTable CacheDT = TDataCache.GetCacheableDataTableFromCache("LanguageCodeList", String.Empty, null, out DataTableType);
      FMainDS.PLanguage.Merge(CacheDT);

      grdDetails.Columns.Clear();
      grdDetails.AddTextColumn("Language Code", FMainDS.PLanguage.ColumnLanguageCode);
      grdDetails.AddTextColumn("Description", FMainDS.PLanguage.ColumnLanguageDescription);
      grdDetails.AddCheckBoxColumn("Congress Language", FMainDS.PLanguage.ColumnCongressLanguage);
      grdDetails.AddCheckBoxColumn("Deletable", FMainDS.PLanguage.ColumnDeletable);

      FPetraUtilsObject.ActionEnablingEvent += ActionEnabledEvent;

      DataView myDataView = FMainDS.PLanguage.DefaultView;
    ...
    /// automatically generated, create a new record of PLanguage and display on the edit screen
    /// we create the table locally, no dataset
    public bool CreateNewPLanguage()
    {
        PLanguageRow NewRow = FMainDS.PLanguage.NewRowTyped();
        NewRowManual(ref NewRow);
        FMainDS.PLanguage.Rows.Add(NewRow);

        FPetraUtilsObject.SetChangedFlag();

        grdDetails.DataSource = new DevAge.ComponentModel.BoundDataView(FMainDS.PLanguage.DefaultView);
        grdDetails.Refresh();
        SelectDetailRowByDataTableIndex(FMainDS.PLanguage.Rows.Count - 1);
    ...
    private void SelectDetailRowByDataTableIndex(Int32 ARowNumberInTable)
    {
        ...
        {
            bool found = true;
            foreach (DataColumn myColumn in FMainDS.PLanguage.PrimaryKey)
            {
                string value1 = FMainDS.PLanguage.Rows[ARowNumberInTable][myColumn].ToString();
    ...
    private PLanguageRow GetSelectedDetailRow()
    {
        DataRowView[] SelectedGridRow = grdDetails.SelectedDataRowsAsDataRowView;

        if (SelectedGridRow.Length >= 1)
        {
            return (PLanguageRow)SelectedGridRow[0].Row;
        }
    ...
    {
        txtDetailLanguageCode.ReadOnly = AReadOnly;
    }
    ...
    private void GetDetailsFromControls(PLanguageRow ARow)
    {
        if (ARow != null)
        {
            ARow.LanguageCode = txtDetailLanguageCode.Text;
            ARow.LanguageDescription = txtDetailLanguageDescription.Text;
            ARow.CongressLanguage = chkDetailCongressLanguage.Checked;
...
//TODO?  still needed?      FMainDS.AApDocument.Rows[0].BeginEdit();
        ...
        {
            foreach (DataRow InspectDR in FMainDS.PLanguage.Rows)
            {
                InspectDR.EndEdit();
            }
            ...
            else
            {
                ...
                Ict.Common.Data.TTypedDataTable SubmitDT = FMainDS.PLanguage.GetChangesTyped();
                ...
                try
                {
                    SubmissionResult = TDataCache.SaveChangedCacheableDataTableToPetraServer("LanguageCodeList", ref SubmitDT, out VerificationResult);
                }
...
*/
                ...
                switch (SubmissionResult)
                {
                    case TSubmitChangesResult.scrOK:
                        ...
                        FMainDS.PLanguage.AcceptChanges();

                        // Merge back with data from the Server (eg. for getting Sequence values)
                        FMainDS.PLanguage.Merge(SubmitDT, false);
