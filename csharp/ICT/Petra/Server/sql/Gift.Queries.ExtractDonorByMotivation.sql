...
FROM pub_a_gift, pub_a_gift_detail, a_motivation_detail, pub_p_partner ##address_filter_tables##
WHERE
        (? OR a_gift_detail.a_ledger_number_i = ?)
    AND (? OR pub_a_gift_detail.p_mailing_code_c = ?)
    AND ((pub_a_gift_detail.a_motivation_group_code_c,pub_a_gift_detail.a_motivation_detail_code_c) IN (##motivation_group_detail_pairs##))
