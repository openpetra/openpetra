    ...
    public class TStewardshipCalculation
    {
                                          ...
                                          )
        {
            ...
            try
            {
                ...
                if (LedgerRow.ProvisionalYearEndFlag)
                {
                    ...
                    ErrorMessage = String.Format(Catalog.GetString("Cannot progress as Ledger {0} is at the provisional year-end point"), ALedgerNumber);
                ...
                if (ProcessedFee.Count > 0)
                {
                    ...
                    AAccountingPeriodTable AccountingPeriodTable = AAccountingPeriodAccess.LoadByPrimaryKey(ALedgerNumber, APeriodNumber, ADBTransaction);
                    ...
                    if (!BatchCreated)
                    {
                        ...
                        ErrorMessage = String.Format(Catalog.GetString("There was a failure while attempting to create the batch for Ledger {0}"), ALedgerNumber);
                        ErrorType = TResultSeverity.Resv_Critical;
                        throw new System.InvalidOperationException(ErrorMessage);
                    }
                    ...
                    if (!JournalCreated) 
                    {
                        //Petra error: GL0035
                        ErrorContext = "Admin Grant & Fee Calculations";
                        ErrorMessage = String.Format(Catalog.GetString("There was a failure while attempting to create the journal for Ledger {0}"), ALedgerNumber);
                        ErrorType = TResultSeverity.Resv_Critical;
                        throw new System.InvalidOperationException(ErrorMessage);
                    }
                    ...
                    for (int i = 0; i < ProcessedFee.Count; i++)
                    {
                        ...
                        if (CurrentFeeCode != pFR.FeeCode)
                        {
                            ...
                            {
                                AFeesReceivableTable FeesReceivableTable =  AFeesReceivableAccess.LoadByPrimaryKey(ALedgerNumber, CurrentFeeCode, ADBTransaction);
                                ...
                                if (FeesReceivableTable.Count == 0)
                                {
                                    ...
                                    ErrorMessage = String.Format(Catalog.GetString("Unable to access information for Fee Code '{1}' in either the Fees Payable & Receivable Tables for Ledger {0}"), ALedgerNumber, CurrentFeeCode);
                                    ErrorType = TResultSeverity.Resv_Critical;
                                    throw new System.InvalidOperationException(ErrorMessage);
                                }
                                else
                                ...
                                else if (pFR2.FeeCode == CurrentFeeCode && pFR2.CostCentreCode != CostCentreCode)
                                {
                                    if (DrFeeTotal != 0)
                                    {
                                        ...
                                        if (!CreatedSuccessfully)
                                        {
                                            ...
                                            ErrorMessage = String.Format(Catalog.GetString("There was a failure attempting to create the transaction for the admin charges."));
                                            ErrorType = TResultSeverity.Resv_Critical;
                                            throw new System.InvalidOperationException(ErrorMessage);
                                        }
                        ...
                        pFR.Timestamp = (DateTime.Today.TimeOfDay.Hours * 3600 + DateTime.Today.TimeOfDay.Minutes * 60 + DateTime.Today.TimeOfDay.Seconds);
                        
                        /* Add the charges on this account to the fee total,
                           creating an entry if necessary. (This is for the income total) */
                        CreditFeeTotalDR = (GLStewardshipCalculationTDSCreditFeeTotalRow)CreditFeeTotalDT.Rows.Find(new object[] {DestCostCentreCode, DestAccountCode}); //, CreditFeeTotalDR.TransactionAmount});
                    ...
                    for (int k = 0; k < CreditFeeTotalDT.Count; k++)
                    {
                        ...
                        if (cFT.TransactionAmount != 0)
                        {
                            ...
                            if (!CreatedSuccessfully)
                            {
                                //Petra error: GL0067
                                ErrorContext = "Create Transactions";
                                ErrorMessage = String.Format(Catalog.GetString("There was a failure attempting to create the transaction for the collected admin charges."));
                                ErrorType = TResultSeverity.Resv_Critical;
                                throw new System.InvalidOperationException(ErrorMessage);
                            }
                        }
                                         ...
                                         )
        {
            ...
            try
            {
                ...
                AAccountingPeriodTable AccountingPeriodTable = AAccountingPeriodAccess.LoadByPrimaryKey(ALedgerNumber, LedgerRow.CurrentPeriod, ADBTransaction);
                ...
                if (AccountingPeriodTable.Count == 0)
                {
                    ...
                    ErrorMessage = String.Format(Catalog.GetString("Accounting Period Key (Ledger = {0}, Accounting Period Number = {1}) does not exist."),
                                                 ALedgerNumber,
                                                 CurrentPeriod
                                                );
                    ErrorType = TResultSeverity.Resv_Noncritical;
                ...
                if (ATestDate > AccountingPeriodRow.PeriodEndDate)
                {
                    ...
                    AccountingPeriodTable = AAccountingPeriodAccess.LoadUsingTemplate(TemplateRow8, operators8, null, ADBTransaction, OrderList8, 0, 0);
                    ...
                    else
                    {
                        ...
                        ErrorContext = "Data Validation";
                        ErrorMessage = String.Format(Catalog.GetString("This date {0} is beyond the current accounting period and the date allowed for forward processing for Ledger: {1}."),
                                                     ATestDate,
                                                     ALedgerNumber);
                        ErrorType = TResultSeverity.Resv_Noncritical;
                        throw new System.InvalidOperationException(ErrorMessage);
            ...
            */
            ...
            try
            {
                ...
                if (CostCentreRow.PostingCostCentreFlag != true) /* {&POSTING} - set to true*/
                {
                    ...
                    ErrorContext = "Cost Centre Validation";
                    ErrorMessage = String.Format(Catalog.GetString("The Cost centre (Ledger = {0}, Code = {1}) is not a posting cost centre. The transaction cannot be posted."),
                                                 ALedgerNumber,
                                                 ACostCentreCode
                                                );
                    ErrorType = TResultSeverity.Resv_Noncritical;
                ...
                if (AccountRow.BudgetControlFlag)
                {
                    ...
                    if (BudgetTable.Count == 0)
                    {
                        ...
                        ErrorMessage = String.Format(Catalog.GetString("There is no budget information for this account/cost centre combination (Cost Centre = {0} Account code = {1})." +
                ...
                if (!AccountRow.PostingStatus)
                {
                    ...
                    ErrorContext = "Account Validation";
                    ErrorMessage = String.Format(Catalog.GetString("The Account (Ledger = {0}, Code = {1}) is not a posting account. The transaction cannot be posted."),
                                                 ALedgerNumber,
                                                 AAccountCode
                                                );
                    ErrorType = TResultSeverity.Resv_Noncritical;
                                               ...
                                               )
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (LedgerRow.ProvisionalYearEndFlag)
                    {
                        AccountingPeriodTable = AAccountingPeriodAccess.LoadByPrimaryKey(ALedgerNumber, LedgerRow.NumberOfAccountingPeriods, ADBTransaction);
                    }
                    else
                    {
                        AAccountingPeriodTable AccountingPeriodTb = new AAccountingPeriodTable();
                        ...
                        AccountingPeriodTable = AAccountingPeriodAccess.LoadUsingTemplate(TemplateRow9, operators9, null, ADBTransaction, OrderList, 0, 0);
                    }
                    ...
                    CorporateExchangeRateTable = ACorporateExchangeRateAccess.LoadUsingTemplate(TemplateRow0, operators0, null, ADBTransaction, OrderList0, 0, 0);
                    ...
                    if (!CorpExchRateFound)
                    {
                        ...
                        */
                        ...
                        ErrorContext = "Accounting Period";
                        ErrorMessage = String.Format(Catalog.GetString("There is no corporate exchange rate for converting to the international currency Please enter a rate for period {0}."),
                                                       AccountingPeriodRow.AccountingPeriodNumber
                                                    );
                        ErrorType = TResultSeverity.Resv_Noncritical;
                        throw new System.InvalidOperationException(ErrorMessage);
        ...
        try
        {
            ...
            else
            {
                ...
                if (CostCentreTable.Count == 0)
                {
                    ...
                    ErrorContext = "Data Validation";
                    ErrorMessage = String.Format(Catalog.GetString("The cost centre: '{0}' does not exist in the Cost Centre table."), ACostCentreCode);
                    ErrorType = TResultSeverity.Resv_Noncritical;
                    throw new System.InvalidOperationException(ErrorMessage);
                }
                else if (AccountTable.Count == 0)
                {
                    ...
                    ErrorContext = "Data Validation";
                    ErrorMessage = String.Format(Catalog.GetString("The account: '{0}' does not exist in the Account Master table.") ,AAccountCode);
                    ErrorType = TResultSeverity.Resv_Noncritical;
                    throw new System.InvalidOperationException(ErrorMessage);
                }
                ...
                if (!CostCentreRow.CostCentreActiveFlag)
                {
                    ...
                    ErrorContext = "Data Validation";
                    ErrorMessage = String.Format(Catalog.GetString("The code: '{0}' is no longer active. Do you still want to use it?"), CostCentreCode);
                    ErrorType = TResultSeverity.Resv_Noncritical;
                    throw new System.InvalidOperationException(ErrorMessage);
                }
                else if (!AccountRow.AccountActiveFlag)
                {
                    ...
                    ErrorContext = "Data Validation";
                    ErrorMessage = String.Format(Catalog.GetString("The code: '{0}' is no longer active. Do you still want to use it?"), AccountCode);
                    ErrorType = TResultSeverity.Resv_Noncritical;
                    throw new System.InvalidOperationException(ErrorMessage);
                }
                    ...
                    if (aRow.ForeignCurrencyFlag)
                    {
                        if (JournalTable.Count > 0)
                        {
                                    ...
                                    && AccountRow.ForeignCurrencyCode != JournalRow.TransactionCurrency)
                            {
                                ...
                                //RUN s_errmsg.p ("GL0055":U,PROGRAM-NAME(1),"Current Amount","").
                                ErrorContext = "Data Validation";
                                ErrorMessage = String.Format(Catalog.GetString("Account {0} cannot be used because it is a foreign currency account. Create a journal using {1} instead."),
                                                               AAccountCode,
                ...
                if (IsValid)
                {
                    if (CostCentreRow.ProjectStatus)
                    {
                        ...
                        AGeneralLedgerMasterTable GeneralLedgerMasterTable = AGeneralLedgerMasterAccess.LoadUsingTemplate(TemplateRow3, operators3, null, ADBTransaction);
                        ...
                        if (GeneralLedgerMasterTable.Count > 0)
                        {
                            ...
                            if (CostCentreRow.ProjectConstraintAmount < CurrentAmount)
                            {
                               ...
                               //Project will exceed constraint
                                ...
                                ErrorContext = "Data Validation";
                                ErrorMessage = String.Format(Catalog.GetString("This transaction will exceed the given project financial constraint for: '{0}' " +
                                                                               "(Limit: {1} Current: {2})" + 
                            ...
                            if (CostCentreRow.ProjectConstraintDate < ATransactionDate)
                            {
                                ...
                                ErrorContext = "Data Validation";
                                ErrorMessage = String.Format(Catalog.GetString("This transaction will exceed the given project time constraint for: '{0}' " +
...
//                                  throw new System.InvalidOperationException(ErrorMessage);
                            ...
                            if (AAccountCode != TransRow.AccountCode)
                            {
                                ...
                                ATransAnalAttribTable TransAnalAttribTable = ATransAnalAttribAccess.LoadUsingTemplate(TemplateRow4, operators4, null, ADBTransaction);
...
//                                  throw new System.InvalidOperationException(ErrorMessage);
                ...
                if (IsValid)
                {
                    ...
                    AAnalysisAttributeTable AnalysisAttributeTable = AAnalysisAttributeAccess.LoadUsingTemplate(TemplateRow, operators, null, ADBTransaction);
                        ...
                        for (int i = 0; i < AnalysisAttributeTable.Count; i++)
                        {
                            ...
                            if (!AssignAttributeOK)
                            {
                              ...
                              //The required analysis attribute &1 has not been entered for account &2 Transaction has not been created.
                                ErrorContext = "Transaction Creation";
                                ErrorMessage = String.Format(Catalog.GetString("The required analysis attribute: {0} has not been entered for account: {1}." + 
...
//        }
                                          ...
                                          )
        {
            ...
            try
            {
                ...
                AAccountingPeriodTable AccountingPeriodTable = AAccountingPeriodAccess.LoadByPrimaryKey(ALedgerNumber, BatchRow.BatchPeriod, ADBTransaction);
                AAccountingPeriodRow AccountingPeriodRow = (AAccountingPeriodRow)AccountingPeriodTable.Rows[0];
                                         ...
                                         )
        {
           ...
           //Return value
            ...
            try
            {
                AAccountingPeriodTable AccountingPeriodTable2 = AAccountingPeriodAccess.LoadByPrimaryKey(ALedgerNumber, ACurrentPeriod, ADBTransaction);
                ...
                else if (ATestDate > AccountingPeriodRow2.PeriodEndDate)
                {
                    ...
                    AAccountingPeriodTable AccountingPeriodTable4 = AAccountingPeriodAccess.LoadUsingTemplate(TemplateRow2, operators2, null, ADBTransaction);
    ...
    //                                    RUN s_errmsg.p ("X_0007":U,
            ...
            {
                ErrorContext = "Date Validation";
                ErrorMessage = String.Format(Catalog.GetString("Unknown error while processing date validation for Accounting Period Number: {0} and Ledger: {1}" +
                                                                Environment.NewLine + Environment.NewLine + ex.ToString()),
                                             ACurrentPeriod,
                                             ALedgerNumber
                                            );
                                                       ...
                                                       )
        {
            ...
            try
            {
                ...
                if (CurrencyTableFrom.Count == 0 || CurrencyTableTo.Count == 0)
                {
                    ...
                    {
                        ErrorContext = "Convert Between European Currencies";
                        ErrorMessage = String.Format(Catalog.GetString("Unable to access the record for currency: {0} in the currencies table."), ACurrencyTo);
                        ErrorType = TResultSeverity.Resv_Noncritical;
                        throw new System.InvalidOperationException(ErrorMessage);
                    }
                    else
                    {
                        ...
                        OutputAmount = Math.Round(AInputAmount/AExchangeRate, THelperNumeric.CalcNumericFormatDecimalPlaces(CurrencyRowTo.DisplayFormat));
                    }
                }
                else /* EMU conversion required*/
                {
                    ...
                    if (CurrencyFrom == MFinanceConstants.EURO_CURRENCY.ToUpper())
                    {
                        ...
                        {
                            ErrorContext = "Convert Between European Currencies";
                            ErrorMessage = String.Format(Catalog.GetString("Unable to access the daily exchange rate for currency: {0} in the exchange rate table."), ACurrencyTo);
                            ErrorType = TResultSeverity.Resv_Noncritical;
                            throw new System.InvalidOperationException(ErrorMessage);
                        }
                        else
                               ...
                               )
        {
            ...
            try
            {
                ...
                ADailyExchangeRateTable DailyExchangeRateTable = ADailyExchangeRateAccess.LoadUsingTemplate(TemplateRow, operators, null, ADBTransaction, OrderList, 0, 0);
                ...
                else
                {
                    ...
                    ErrorMessage = String.Format(Catalog.GetString("Rate for converting from {0} to {1} does not exist. Do you wish to add it?"), ACurrencyFrom, ACurrencyTo);
                    ErrorType = TResultSeverity.Resv_Noncritical;
                                 ...
                                 )
        {
            ...
            AGeneralLedgerMasterTable GeneralLedgerMasterTable = AGeneralLedgerMasterAccess.LoadByUniqueKey(ALedgerNumber, AYear, AAccountCode, ACostCentreCode, ADBTransaction);
                                        ...
                                        )
        {
            ...
            else if (APeriodNumber > ANumberOfAccountingPeriods)
            {
                ...
                {
                    GeneralLedgerMasterPeriodTable = AGeneralLedgerMasterPeriodAccess.LoadByPrimaryKey(AGlmSequenceThisYear, APeriodNumber, ADBTransaction);
                }
                else
                {
                    GeneralLedgerMasterPeriodTable = AGeneralLedgerMasterPeriodAccess.LoadByPrimaryKey(AGlmSequenceNextYear, (APeriodNumber - ANumberOfAccountingPeriods), ADBTransaction);
                }
            }
            else
            {
