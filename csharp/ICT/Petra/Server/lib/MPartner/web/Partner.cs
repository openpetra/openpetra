        ...
        public static bool DeletePartner(Int64 APartnerKey, out TVerificationResultCollection AVerificationResult)
        {
            ...
            if (ResultValue)
            {
                ...
                foreach (DataRow Row in PartnerLocationTable.Rows)
                {
                    ...
                    OtherPartnerLocationTable = PPartnerLocationAccess.LoadViaPLocation(PartnerLocationRow.SiteKey, PartnerLocationRow.LocationKey, Transaction);
            ...
            if (ResultValue)
            {
                ...
                foreach (DataRow Row in PartnerBankingDetailsTable.Rows)
                {
                    ...
                    OtherPartnerBankingDetailsTable = PPartnerBankingDetailsAccess.LoadViaPBankingDetails(PartnerBankingDetailsRow.BankingDetailsKey, Transaction);
            ...
            if (ResultValue)
            {
                ...
                {
                    TLogging.Log("Problem during deletion of " + PPartnerContactAttributeTable.GetTableDBName() + "while deleting a partner: " + e.Message);
                    ResultValue = false;
                }
            }
