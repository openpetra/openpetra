    ...
    public class TPartnerFileExport : TImportExportTextFile
    {
        ...
        private void WriteShortApplicationForm(PartnerImportExportTDS AMainDS, PmGeneralApplicationRow AGeneralApplicationRow)
        {
            ...
            if (AMainDS.PmShortTermApplication.DefaultView.Count > 0)
            {
                ...
                Write(ShortTermApplicationRow.IsStLeadershipRatingNull()? "" : ShortTermApplicationRow.StLeadershipRating);
        ...
        private void WriteLongApplicationForm(PartnerImportExportTDS AMainDS, PmGeneralApplicationRow AGeneralApplicationRow)
        {
            ...
            if (AMainDS.PmYearProgramApplication.DefaultView.Count > 0)
            {
                ...
                Write(Row.IsYpAppFeeReceivedNull()? false : Row.YpAppFeeReceived);
        ...
        private void WriteApplications(PartnerImportExportTDS AMainDS)
        {
            foreach (PmGeneralApplicationRow GeneralApplicationRow in AMainDS.PmGeneralApplication.Rows)
            {
                if (!GeneralApplicationRow.GenAppDeleteFlag)
                {
                    ...
                    Write(GeneralApplicationRow.IsGenAppRecvgFldAcceptNull()? "?" : GeneralApplicationRow.GenAppRecvgFldAccept.Value.ToString(DATEFORMAT));
                    Write(GeneralApplicationRow.IsGenAppSrvFldAcceptNull()? false : GeneralApplicationRow.GenAppSrvFldAccept);
                    Write(GeneralApplicationRow.IsGenAppSendFldAcceptDateNull()? "?" : GeneralApplicationRow.GenAppSendFldAcceptDate.Value.ToString(DATEFORMAT));
                    ...
                    foreach (DataRowView v in AMainDS.PmApplicationForms.DefaultView)
                    {
                        ...
                        Write("APPL-FORM");
                        Write(ApplicationFormRow.IsFormNameNull()? "" : ApplicationFormRow.FormName);
                        WriteLine();
                        Write(ApplicationFormRow.IsFormDeleteFlagNull()? false : ApplicationFormRow.FormDeleteFlag);
                        Write(ApplicationFormRow.IsFormEditedNull()? false : ApplicationFormRow.FormEdited);
                        Write(ApplicationFormRow.IsFormReceivedDateNull()? "?" : ApplicationFormRow.FormReceivedDate.Value.ToString(DATEFORMAT));
                        Write(ApplicationFormRow.IsFormReceivedNull()? false : ApplicationFormRow.FormReceived);
                        Write(ApplicationFormRow.IsFormSentDateNull()? "?" : ApplicationFormRow.FormSentDate.Value.ToString(DATEFORMAT));
                        Write(ApplicationFormRow.IsFormSentNull()? false : ApplicationFormRow.FormSent);
                        WriteLine();
                        Write(ApplicationFormRow.IsReferencePartnerKeyNull()? 0 : ApplicationFormRow.ReferencePartnerKey);
                        Write(ApplicationFormRow.IsCommentNull()? "" : ApplicationFormRow.Comment);
                        WriteLine();
                    }
        ...
        private void WritePersonnelData(PartnerImportExportTDS AMainDS)
        {
            ...
            foreach (PmSpecialNeedRow SpecialNeedRow in AMainDS.PmSpecialNeed.Rows)
            {
                ...
                Write(SpecialNeedRow.IsContactHomeOfficeNull()? false : SpecialNeedRow.ContactHomeOffice);
            ...
            foreach (PmPersonVisionRow PersonVisionRow in AMainDS.PmPersonVision.Rows)
            {
                Write("VISION");
                WriteLine();
                Write(PersonVisionRow.IsVisionAreaNameNull()? "" : PersonVisionRow.VisionAreaName);
                WriteLine();
                Write(PersonVisionRow.IsVisionLevelNull()? 0 : PersonVisionRow.VisionLevel);
                Write(PersonVisionRow.IsVisionCommentNull()? "" : PersonVisionRow.VisionComment);
                WriteLine();
            }
