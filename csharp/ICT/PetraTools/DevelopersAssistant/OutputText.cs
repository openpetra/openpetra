...
namespace Ict.Tools.DevelopersAssistant
{
    ...
    class OutputText
    {
        ...
        /// </summary>
        static public string ConciseOutput { get { return _conciseOutput; } }
        /// <summary>
        /// Gets the verbose output string
        /// </summary>
        static public string VerboseOutput { get { return _verboseOutput; } }
        ...
        {
            if (o == OutputStream.Concise || o == OutputStream.Both) _conciseOutput += s;
            if (o == OutputStream.Verbose || o == OutputStream.Both) _verboseOutput += s;
        }
        ...
        static public void AddLogFileOutput(string path, ref int NumFailed, ref int NumWarnings)
        {
            ...
            else
            {
                ...
                AppendText(OutputStream.Both, String.Format("~~~~~~~ {0} succeeded, {1} failed, {2} warnings\r\n\r\n", NumSucceeded, NumFailed, NumWarnings));
        ...
        public static List<ErrorItem> FindWarnings()
        {
            ...
            while (pWarning >= 0 || pError >= 0)
            {
                ...
                if (pWarning >= 0) pWarning = _verboseOutput.IndexOf("warning ", pWarning, StringComparison.InvariantCultureIgnoreCase);
                if (pError >= 0) pError = _verboseOutput.IndexOf("BUILD FAILED", pError, StringComparison.InvariantCultureIgnoreCase);
                if (pWarning >= 0 && pError >= 0)
                {
                    // we have both
                    if (pWarning < pError) bAddWarning = true;
                    else bAddError = true;
                }
                else if (pWarning >= 0)
                {
