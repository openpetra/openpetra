...
namespace Ict.Tools.DevelopersAssistant
{
    class ClientAutoLogOn
    {
        ...
        public string TestAction { get { return _testAction; } }
        ...
        public bool ResetConfig()
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("The Assistant could not find the repository copy of the standard configuration file.", Program.APP_TITLE);
                return false;
            }
            try
            {
        ...
        public bool UpdateConfig(bool UseAutoLogon, string UserName, string Password, string TestAction)
        {
            ...
            try
            {
                if (!File.Exists(path))
                {
                    ...
                    {
                        System.Windows.Forms.MessageBox.Show("Failed to create your personal client configuration because the standard repository file could not be found.", Program.APP_TITLE);
                        return false;
                    }
                }
                ...
                if (TestAction == String.Empty) RemoveProperty(xmlDoc, "TestAction"); else SetPropertyValue(xmlDoc, "TestAction", TestAction);
                xmlDoc.Save(path);
            }
            catch (Exception ex)
            {
