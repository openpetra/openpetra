...
namespace WindowsFormsSample
{
	...
	{
		private SourceGrid.DataGrid grid1;
		...
		private void frmSample17_Load(object sender, System.EventArgs e)
		{
			...
			{			
				tab.Columns.Add(new DataColumn("col " + i.ToString()));
			}
			...
			{			
				...
				for (int i = 0; i < columns; i++)
				{			
					tab.Rows[rowCounter][i] = (char)('A' + rowCounter) + i.ToString();
				}
			}
			grid1.DataSource = new DevAge.ComponentModel.BoundDataView(tab.DefaultView);
			for (int i = 0; i < columns; i++)
			{
				grid1.Columns.Add(new SourceGrid.DataGridColumn(grid1));
			}
