...
namespace SourceGrid.Examples.Threading
{
	...
	public abstract class AsynchroniousOperation
	{
		/// <summary>
		...
		/// correct thread. Usually this is a Form</param>
		public AsynchroniousOperation(ISynchronizeInvoke target)
		{
			isiTarget = target;
			isRunning = false;
		}
		...
		private void InternalStart()
		{
			...
			catch (Exception e)
			{
				...
				try
				{
					...
					FailOperation(
						new AsyncOperationException(string.Format(
							"Error executing asynchronious command {0} ",
