...
namespace SourceGrid.Cells.Controllers
{
	public class ControllerContainer : IController
	{
		...
		public void OnMouseMove(CellContext sender, System.Windows.Forms.MouseEventArgs e)
		{
            foreach (IController controller in m_ControllerList)
            {
                // AlanP: Jan 2016.  We only deal with OnMouseMove for the resizable headers
                // There is a problem for OP if we handle OnMouseMove for Mouse Grid Selection because if OP puts up a message box
                // during Validation (which it does if there are Warnings but no errors) events come through in the wrong order
                // and after closing the dialog moving the mouse turns rows blue (highlighted).
                if (controller is Resizable || controller is CellEventDispatcher)
                {
