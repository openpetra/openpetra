...
namespace SourceGrid.Selection
{
	...
	public abstract class SelectionBase : IGridSelection
	{
        ...
        // AlanP: Sep 2013  Added a protected member variable that can be set to indocate that selection_changed is suppressed
        protected bool m_SuppressSelectionChangedEvent = false;
		...
		{
			//Deselect previous selected cells
            // AlanP: Sep 2013.  I have commented this line back in.  The problem that it caused occurred when the
            //   Focus method was called for a row that was already the active position and when the other rows were to be reset.
            //   This could happen after deleting multiple rows in our grid.
            // In this case the other rows are not reset by Focus(), so we need to reset them here...
			ResetSelection(true);
			//Select the cell
		...
		public bool MoveActiveCell(Position start, int rowShift, int colShift, bool resetSelection)
		{
			...
			// AlanP: Sep 2013  Suppress selection_changed if SHIFT is pressed - we will still get another chance to raise it
            if (newPosition.IsEmpty() == false)
				return Focus(newPosition, resetSelection, !resetSelection);
			else
				return false;
		}
