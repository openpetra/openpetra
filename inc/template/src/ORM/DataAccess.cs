...
public class {#TABLENAME}Access : TTypedDataAccess
{
    ...
    {
        DBAccess.GetDBAccessObj(ATransaction).Select(ADataSet, ((GenerateSelectClause(AFieldList, {#TABLENAME}Table.TableId) + " FROM PUB_{#SQLTABLENAME}") +
    ...
    public static {#TABLENAME}Table LoadAll(StringCollection AFieldList, TDBTransaction ATransaction, StringCollection AOrderBy, int AStartRecord, int AMaxRecords)
    {
        ...
        DBAccess.GetDBAccessObj(ATransaction).SelectDT(Data, GenerateSelectClause(AFieldList, {#TABLENAME}Table.TableId) + " FROM PUB_{#SQLTABLENAME}" + GenerateOrderByClause(AOrderBy), ATransaction, null, AStartRecord, AMaxRecords);
        return Data;
    }
...
{#ENDIF FORMALPARAMETERSUNIQUEKEY}
    ...
    {
        return Convert.ToInt32(DBAccess.GetDBAccessObj(ATransaction).ExecuteScalar("SELECT COUNT(*) FROM PUB_{#SQLTABLENAME}", ATransaction));
    }
{#IFDEF FORMALPARAMETERSPRIMARYKEY}
...
{#ENDIF FORMALPARAMETERSUNIQUEKEY}
    ...
    {
        return Convert.ToInt32(DBAccess.GetDBAccessObj(ATransaction).ExecuteScalar(("SELECT COUNT(*) FROM PUB_{#SQLTABLENAME}" + GenerateWhereClause(TTypedDataTable.GetColumnStringList({#TABLENAME}Table.TableId), ATemplateRow, ATemplateOperators)), ATransaction, 
    ...
    {
        return Convert.ToInt32(DBAccess.GetDBAccessObj(ATransaction).ExecuteScalar(("SELECT COUNT(*) FROM PUB_{#SQLTABLENAME}" + GenerateWhereClause(ASearchCriteria)), ATransaction, 
...
public static void Load{#VIAPROCEDURENAME}(DataSet ADataSet, {#FORMALPARAMETERSOTHERPRIMARYKEY}, StringCollection AFieldList, TDBTransaction ATransaction, StringCollection AOrderBy, int AStartRecord, int AMaxRecords)
{
    ...
    DBAccess.GetDBAccessObj(ATransaction).Select(ADataSet, ((GenerateSelectClauseLong("PUB_{#SQLTABLENAME}", AFieldList, {#TABLENAME}Table.TableId) + 
...
{
    {#ODBCPARAMETERSFOREIGNKEY}
    return Convert.ToInt32(DBAccess.GetDBAccessObj(ATransaction).ExecuteScalar("SELECT COUNT(*) FROM PUB_{#SQLTABLENAME}, PUB_{#SQLLINKTABLENAME} WHERE " +
                "{#WHERECLAUSEVIALINKTABLE}",
...
public static int Count{#VIAPROCEDURENAME}Template({#OTHERTABLENAME}Row ATemplateRow, StringCollection ATemplateOperators, TDBTransaction ATransaction)
{
    return Convert.ToInt32(DBAccess.GetDBAccessObj(ATransaction).ExecuteScalar(("SELECT COUNT(*) FROM PUB_{#SQLTABLENAME}, PUB_{#SQLLINKTABLENAME}, PUB_{#SQLOTHERTABLENAME} WHERE " +
                "{#WHERECLAUSEALLVIATABLES}" +
