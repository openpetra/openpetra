...
class MaintainConsentPurposes {
	...
	self.format_item(item) {
		...
		$('#btnEdit'+item['p_purpose_code_c_clean']).on('click', function() {self.open_edit(item['p_purpose_code_c'])});
		$('#btnView'+item['p_purpose_code_c_clean']).on('click', function() {self.open_detail($(this))});
	}

	open_detail(obj) {
		let self = this
		obj = $(obj);
		while(!obj[0].hasAttribute('id') || !obj[0].id.includes("purpose")) {
			obj = obj.parent();
		}
...
export default new MaintainConsentPurposes();