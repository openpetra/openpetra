...
class MaintainMemberships {
	...
	display_list() {
		...
		data = JSON.parse(data.data.d);
			...
			self.last_requested_data = data.result.PMembership;
			if (!data.result.PMembership) { return }
			for (var item of data.result.PMembership) {
	...
	self.format_item(item) {
		...
		$('#btnEdit'+item['p_membership_code_c_clean']).on('click', function() {self.open_edit(item['p_membership_code_c'])});
		$('#btnView'+item['p_membership_code_c_clean']).on('click', function() {self.open_detail($(this))});
	}
...
export default new MaintainMemberships();