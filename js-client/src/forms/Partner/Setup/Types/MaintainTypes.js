...
class MaintainTypes {
			...
			data = JSON.parse(data.data.d);
				...
				for (var item of data.result.PType) {
					...
					if (item['p_category_code_c'] != null) {
						item['combined_code'] = item['p_category_code_c'] + " / " + item['p_type_code_c']
					} else {
						item['combined_code'] = item['p_type_code_c']
					}
	...
	self.format_item(item) {
		...
		$('#btnEdit'+item['p_type_code_c_clean']).on('click', function() {self.open_edit(item['p_type_code_c'])});
		$('#btnView'+item['p_type_code_c_clean']).on('click', function() {self.open_detail($(this))});
	}
...
export default new MaintainTypes();