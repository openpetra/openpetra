...
class RecurringGiftBatches {
		...
		api.post('serverMFinance.asmx/TGiftTransactionWebConnector_LoadARecurringGiftBatch', x).then(function (data) {
			...
			if (window.location.href.includes('?ledger_number=')) {
				...
				if (window.localStorage.getItem('current_ledger') == ledger_number) {
					self.open_gift_transactions($('#Batch' + batch_number), batch_number, true, gift_transaction_number);
				}
	...
	updateBatch(BatchNumber) {
		...
		api.post('serverMFinance.asmx/TGiftTransactionWebConnector_LoadARecurringGiftBatchSingle', x).then(function (data) {
			...
			tpl.format_currency(item.a_currency_code_c);
			tpl.format_date();
			self.open_gift_transactions($('#Batch' + BatchNumber), BatchNumber, true);
		});
	}
	...
	new_trans(ledger_number, batch_number) {
		let self = this;
		if (!modal.allow_modal()) {return}
		let transaction_number = $("#Batch" + batch_number + " .tpl_gift").length + 1
		let x = {
			a_ledger_number_i: ledger_number,
			a_batch_number_i: batch_number,
			a_gift_transaction_number_i: transaction_number,
		};
		var today = new Date();
	...
	new_trans_detail(btn, ledger_number, batch_number, trans_id) {
		...
		let donorkey = obj.find('input[name=p_donor_name_c]').attr('key-value');
		let donorname = obj.find('input[name=p_donor_name_c]').val().replace(donorkey + " ", '');
		let x = {
			a_ledger_number_i: ledger_number,
			a_batch_number_i: batch_number,
	...
	update_motivation_group(input_field_object, selected_value) {
		...
		if (details.membership) {
			...
			if (recipient.val() == '') {
				...
				let donorname = obj.find('input[name=p_donor_name_c]').val();
	...
	edit_gift_trans(ledger_id, batch_id, trans_id) {
		let self = this;
		if (!modal.allow_modal()) {return}
			...
			for (var detail of parsed.result.ARecurringGiftDetail) {
				if (detail.a_gift_transaction_number_i == trans_id) {
					...
					tpl_trans_detail.find('#btnEditDetail').on('click', function() {self.edit_gift_trans_detail(
						detail.a_ledger_number_i, detail.a_batch_number_i, detail.a_gift_transaction_number_i, detail.a_detail_number_i)});
				}
			}
	...
	install_ContactEdit_and_RefreshAccounts(obj, donorkey) {
		let self = this;
		...
		btnRefreshBankAccounts.click(function() {
			let partnerkey = $(this).attr('partnerkey');
			self.onselect_donor($(this), partnerkey, false);
		});
	}

	onselect_donor(obj, donor_key, reset_selection = true) {
		let self = this;
		...
		api.post('serverMFinance.asmx/TGiftTransactionWebConnector_LoadBankingDetailsOfPartner', x).then(function (data) {
			...
			self.loadbankaccounts(parsed.result.PPartnerBankingDetails, donor_key, selected, obj.closest('.modal'));
			...
			let mandateReferenceID = obj.closest('.modal').find('input[name=a_sepa_mandate_reference_c]');
	...
	loadbankaccounts(accounts_of_donor, donor_key, selected_account, obj) {
		let self = this;
	...
	edit_gift_trans_detail(ledger_id, batch_id, trans_id, detail_id) {
		let self = this;
		if (!modal.allow_modal()) {return}
	...
	save_edit_trans_detail(obj_modal) {
		...
		api.post('serverMFinance.asmx/TGiftTransactionWebConnector_MaintainRecurringGiftDetails', payload).then(function (result) {
			...
			if (parsed.result == true) {
				utils.display_message(i18next.t('forms.saved'), "success");
				self.updateGift(payload['ABatchNumber'], payload['AGiftTransactionNumber']);
				self.updateBatch(payload['ABatchNumber']);
			}
			else if (parsed.result == false) {
				utils.display_error(parsed.AVerificationResult);
	...
	submit_batch(obj, batch_id) {
		...
		api.post( 'serverMFinance.asmx/TGiftTransactionWebConnector_SubmitRecurringGiftBatch', x).then(function (data) {
			data = JSON.parse(data.data.d);
			if (data.result == true) {
				utils.display_message( i18next.t('RecurringGiftBatches.success_submit'), 'success' );
	...
	export_batch(batch_id) {
		...
		api.post('serverMFinance.asmx/TGiftTransactionWebConnector_ExportAllRecurringGiftBatchData', r).then(function (result) {
			let parsed = JSON.parse(result.data.d);
			if (parsed.result > 0) {
				let excelfile = parsed.AExportExcel;
	...
	download_sepa(obj, batch_id) {
		...
		api.post('serverMFinance.asmx/TGiftTransactionWebConnector_ExportRecurringGiftBatch', r).then(function (result) {
			let parsed = JSON.parse(result.data.d);
			if (parsed.result) {
				let sepafile = parsed.ASEPAFileContent;
				...
				utils.display_error(parsed.AMessages);
			}
		});
	}
...
export default new RecurringGiftBatches();